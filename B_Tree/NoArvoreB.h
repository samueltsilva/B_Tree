...
public:
    ...
    void setC2(No_Arvore_B* c, int _pos3, int _k);
    int getN();
    int getChaves(int i);
    bool getFolha();
...
void No_Arvore_B::setChaves(int _pos, int _k) {
    chaves[_pos] = _k;
}
...
No_Arvore_B* No_Arvore_B::procurar(int k) {
    int i = 0; //contador
...
void No_Arvore_B::insert_non_full(int k) {
    int i = n - 1;
...
int No_Arvore_B::encontrar_chave(int k) {
    int aux = 0;
    while (aux < n && chaves[aux] < k)
        aux++;
...
void No_Arvore_B::remover(int k) {
    int aux = encontrar_chave(k);

    if (aux < n && chaves[aux] == k) { //Como visto antes, com o aux, temos a posição onde está a chave
        ...
        if (folha) {
            cout << "A chave " << k << "não está na árvore" << endl;
            return;
...
void No_Arvore_B::remover_nao_folha(int aux) {
    ...
    if (c[aux]->n >= t) { //Se a quantidade de chaves do filho à esquerda for maior que t, podemos pegar a chave antecessora (que está nesse filho) para subirmos para a página que estamos removendo
        int anterior = pegar_predecessor(aux);
        chaves[aux] = anterior;
        ...
        int sucessor = pegar_sucessor(aux);
        chaves[aux] = sucessor;
